...
$$
where $N(\efakey)$ is counted from all reconstructed $\gamma e$ and $N(e\rightsquigarrow e)$ is counted from reconstructed $e^+e^-$, both for 2-body invariant masses around the $Z$-peak. For reconstructed $e^+e^-$, in principle either of the two electrons may fake a photon: for this reason, both $e^\pm$ are counted into $N(e\rightsquigarrow e)$. The \fey\ computation can be carried out also in bins of $|\eta|$ and $p_T$: in this case, the photon in the $\gamma e$ final state, and both electrons in the $e^+e^-$ final state are counted into the $(|\eta|,p_T)$ bin they belong to.
...
However, for $ee$ events both the leading and subleading electrons are used to compute $N(e\rightsquigarrow e)$ and without aligning the selection criteria, the kinematic requirement would be applied correctly only half of the time depending on the electron ordering.
Events are also required to pass a preselection based on a logical $OR$ of missing transverse momentum criteria (see sec~\ref{sec:CR}):
$$E_T^{\text{miss}}> 50 \text{GeV} \parallel E_T^{miss, no-\mu}> 50 \text{GeV}  \parallel  E_T^{miss, no-e}> 50 \text{GeV} $$
An additional offline selection on $E_T^{\text{miss}}$ is applied, as $Z \rightarrow ee$ events are typically back-to-back, making the impact of removing two electrons approximately zero. \\
...
This approach turned out not to be convenient for this study, the reasons being the large drop in the statistics and a misalignment of the $\gamma \mu$ channel caused by the $E_T^{miss, no-\mu}$ selection. This method is investigated in detail in Appendix~\ref{sec:efy-appendix}. \\
...
\begin{figure} [H]%[!htbp] t!
    \centering
    \includegraphics[width=0.7\textwidth]{../images/dark_photon/images/plot_efy/closure/canvas_fey_trigger_MC_ye_minv_mTcut000_etabin00_ptbin00.pdf}
    \caption{Invariant mass distribution in the $\gamma e$ channel.}
    \label{fig:MCye}
\end{figure}
\begin{figure} [H]%[!htbp] t!
    \centering
    \includegraphics[width=0.7\textwidth]{../images/dark_photon/images/plot_efy/closure/canvas_fey_trigger_MC_ymu_minv_mTcut000_etabin00_ptbin00.pdf}
    \caption{Invariant mass distribution in the $\gamma \mu$ channel}
    \label{fig:MCyu}
\end{figure}
\begin{figure} [H]%[!htbp] t!
    \centering
    \includegraphics[width=0.7\textwidth]{../images/dark_photon/images/plot_efy/closure/canvas_fey_trigger_MC_ee_minv_mTcut000_etabin00_ptbin00.pdf}
...
\begin{figure} [H]%[!htbp] t!
  ...
  \includegraphics[width=0.7\textwidth]{../images/dark_photon/images/plot_efy/fake_rate/2024/canvas_fey_trigger_minv_mTcut000_side_both_syst_nominal_etabin00_ptbin00.pdf}
...
\begin{figure}[htbp]
  \centering
  \subfloat[]{
    \includegraphics[width=0.7\textwidth]{../images/dark_photon/images/plot_efy/fake_rate/2024/canvas_fey_trigger_minv_mTcut000_side_low_syst_nominal_etabin00_ptbin00.pdf}%
  }
  \\ %\hfill
  \subfloat[]{
    \includegraphics[width=0.7\textwidth]{../images/dark_photon/images/plot_efy/fake_rate/2024/canvas_fey_trigger_minv_mTcut000_side_upp_syst_nominal_etabin00_ptbin00.pdf}%
  }
  \caption{Invariant mass distribution of $\gamma e$ events and $\gamma \mu$ background, normalized using only the lower tail (top) or the upper tail (bottom).}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \subfloat[]{
    \includegraphics[width=0.7\textwidth]{../images/dark_photon/images/plot_efy/fake_rate/2024/canvas_fey_trigger_minv_mTcut000_side_both_syst_peak_width_etabin00_ptbin00.pdf}%
  }
  \\ %\hfill
  \subfloat[]{
    \includegraphics[width=0.7\textwidth]{../images/dark_photon/images/plot_efy/fake_rate/2024/canvas_fey_trigger_minv_mTcut000_side_both_syst_tail_length_etabin00_ptbin00.pdf}%
  }
  \caption{Invariant mass distribution of $\gamma e$ events and $\gamma \mu$ background, using the varied definition of peak region (top) and sidebands (bottom).}
\end{figure}
...
As a result, $p_T$-inclusive fake rates are computed and presented in Table~\ref{tab:efy-results-2023} and~\ref{tab:efy-results-2024}, for years 2023 and 2024, yielding a lower overall uncertainty.
...
%    \end{tabular}
%    \caption{Fake rates for exclusive bins of $[\eta, p_T]$ and the total error. The different contributions of stati%stical and systematic errors are shown.}
%        \label{tab:efy-binned}
%\end{table}

\begin{figure} [H]%[!htbp] t!
    \centering
    % \includegraphics[width=0.8\textwidth]{plot_efy/fake_rate/OLD/canvas_fey_trigger_vs_pt.pdf}
    \includegraphics[width=0.45\textwidth]{../images/dark_photon/images/plot_efy/fake_rate/2023/canvas_fey_trigger_vs_pt.pdf}
    \hfill
    \includegraphics[width=0.45\textwidth]{../images/dark_photon/images/plot_efy/fake_rate/2024/canvas_fey_trigger_vs_pt.pdf}
    \caption{Fake rate as a function of $p_T$ in different $\eta$ bins: for year 2023 and 2024, respectively on the left and on the right.}
    \label{fig:ff-efy}
\end{figure}
...
\begin{table}
  \centering
  \begin{tabular}{|c|c|c|c|c|c|c|} \hline
    etabin & fey & stat & peak & tail & side & rel err  \\ \hline
etabin01 & $0.02695 \pm 0.00179$ & 0.00172  & 0.00000  & 0.00000  & 0.00051 & 0.067 \\
etabin02 & $0.02822 \pm 0.00168$ & 0.00161  & 0.00000  & 0.00000  & 0.00049 & 0.059 \\
etabin04 & $0.05412 \pm 0.00422$ & 0.00361  & 0.00000  & 0.00000  & 0.00218 & 0.078 \\
etabin05 & $0.10188 \pm 0.00478$ & 0.00430  & 0.00145  & 0.00000  & 0.00148 & 0.047 \\
    \hline
  \end{tabular}
  \caption{Fake rates and the total error for different $\eta$ bins and inclusive binning in $p_T$, for year 2023. The different contributions of statistical and systematic errors are shown.}
  \label{tab:efy-results-2023}
\end{table}
...
\begin{table}
  ...
  \begin{tabular}{|c|c|c|c|c|c|c|} \hline
    etabin & fey & stat & peak & tail & side & rel err  \\ \hline
etabin01 & $0.03734 \pm 0.00135$ & 0.00082  & 0.00098  & 0.00000  & 0.00046 & 0.036 \\
etabin02 & $0.02868 \pm 0.00103$ & 0.00071  & 0.00064  & 0.00000  & 0.00038 & 0.036 \\
etabin04 & $0.04860 \pm 0.00395$ & 0.00160  & 0.00311  & 0.00000  & 0.00182 & 0.081 \\
etabin05 & $0.09252 \pm 0.00298$ & 0.00183  & 0.00000  & 0.00000  & 0.00235 & 0.032 \\
    \hline
  \end{tabular}
  \caption{Fake rates and the total error for different $\eta$ bins and inclusive binning in $p_T$, for year 2024. The different contributions of statistical and systematic errors are shown.}
  \label{tab:efy-results-2024}
\end{table}
...
The SR and the probe-e CR are both defined by applying the analysis trigger in order to have a similar topology of the phase space (see~\ref{sec:fakephotonintro}) and the events in the probe-e CR are required to pass a single electron trigger in order to be consistent with the fake rate evaluation.
...
The analysis trigger efficiency on $\gamma$-objects can be evaluated using a $\gamma$-channel or a $\gamma \mu$-channel, selected with a single-$\mu$ baseline trigger and applying the offline cuts on $E_T^{miss, no \mu}$ and $ m_T(\gamma,E_T^{miss, no \mu})$, to treat the muon as invisible.
Its effect on $e$-objects is computed in a $e$-channel, selected with a single-$e$ baseline trigger and offline cuts on $E_T^{\text{miss}}$ and $ m_T(e,E_T^{\text{miss}})$. \\
Higher offline selections are applied on $E_T^{\text{miss}}$ and $m_T$, in order to cover the trigger impact and isolate it's effect on the $\gamma$ or $e$.
Several configurations of increasing thresholds are tested, trying to maximize trigger efficiency $\epsilon(p_T)$:
\begin{itemize}
     \item $E_T^{\text{miss}} > $ 70, 100, 150, 200, 300, 400 GeV
...
\begin{tabular}{|c|c|c|}
  \hline
  Channel & $E_T^{\text{miss}}$ cut & $m_T$ cut \\
  \hline
...
\begin{figure}[htbp]
  ...
  \subfloat[$\gamma\mu$ channel]{
    \includegraphics[width=0.5\textwidth]{../images/dark_photon/images/plot_efy/trigger/2024/canvas_fey_eff_anatrig_vs_pT_ymu_pTmiss150_mT100.pdf}
  }
  \\ %\hfill
  \subfloat[$\gamma$ channel]{
    \includegraphics[width=0.5\textwidth]{../images/dark_photon/images/plot_efy/trigger/2024/canvas_fey_eff_anatrig_vs_pT_y_pTmiss300_mT100.pdf}
  }
  \\ %\hfill
  \subfloat[$e$ channel]{
    \includegraphics[width=0.5\textwidth]{../images/dark_photon/images/plot_efy/trigger/2024/canvas_fey_eff_anatrig_vs_pT_e_pTmiss200_mT100.pdf}
  }
  \caption{Trigger efficiency as a function of transverse momentum in the $\gamma\mu$, $\gamma$, and $e$ channels when the corresponding best cuts on $E_T^{miss},~m_T$ are applied.
    In each figure, the top panel shows the events distribution before (black) and after (red) applying the analysis trigger. The trigger efficiency (in blue, in the bottom panel) is computed as the ratio of the distributions in the top panel. The vertical red line at 50~GeV marks the offline cut on the photon $p_T$.
  }
  \label{fig:eff-best}
\end{figure}
...
The trigger efficiency on electrons is affected only by the selections on $E_T^{\text{miss}}$ and $m_T$ and the variations in the selections are applied by tightening the selection thresholds.
For photons, three sources of systematic uncertainty are considered: the selections on $E_T^{\text{miss}}$ and $m_T$, as well as the choice of the channel $\gamma \mu$ used to compute $\epsilon_{\gamma}(p_T)$. 
...
\begin{table}[htbp]
  ...
  $\epsilon_{\gamma}(p_T)$ & Channel & $\gamma\mu$ & $\gamma$ \\
                           & $E_T^{miss, \text{no } \mu}$ & 150 GeV &  200 GeV \\
                           & $m_T(\gamma, E_T^{miss, \text{no } \mu})$ & 100 GeV &  150 GeV \\
  \hline
  $\epsilon_{e}(p_T)$      & $E_T^{\text{miss}}$ &  200 GeV &  250 GeV \\
                           & $m_T(e, E_T^{\text{miss}})$ &  100 GeV &  150 GeV \\
  \hline
  \end{tabular}
...
The computed scale factors and total errors as a function of $p_T$ are displayed in Figure~\ref{fig:fey_trigger_scf} and reported in Tables~\ref{tab:scf-results-2023} and~\ref{tab:scf-results-2024} for years 2023 and 2024.

\begin{figure}{h}
  \centering
  \includegraphics[width=0.45\columnwidth]{../images/dark_photon/images/plot_efy/trigger/2023/canvas_fey_scf_anatrig_vs_pT.pdf}
  \hfill
  \includegraphics[width=0.45\columnwidth]{../images/dark_photon/images/plot_efy/trigger/2024/canvas_fey_scf_anatrig_vs_pT.pdf}
  \caption{Trigger scale factors for the \efakey\ estimates: for year 2023 (on the left) and year 2024 (on the right). The vertical red line at 50~GeV marks the offline cut on the photon $p_T$.}
  \label{fig:fey_trigger_scf}
\end{figure}
...
%      \hline
%      $p_T$ [GeV] & scf $\pm$ tot err & stat & channel & $p_T^{\text{miss}}(\gamma\mu)$ & $m_T(\gamma\mu)$ &  $p_T^{\text{miss}}(e)$ & $m_T(e)$ & rel err \\
%      \hline
...
\begin{table}[h]
  ...
  \begin{tabular}{|c|c|c|c|c|c|c|c|}
    \hline
    $p_T$ [GeV] & scf $\pm$ tot err & stat & channel & $E_T^{\text{miss}}(\gamma\mu)$ & $m_T(\gamma\mu)$ &  $E_T^{\text{miss}}(e)$ & $m_T(e)$ \\
    \hline
    $[50000;52000]$ & $1.257(+/-)0.177$ & 0.102 &  0.096 & 0.109 & 0.000 & 0.000 &  0.000 \\
    $[52000;54000]$ & $1.040(+/-)0.195$ & 0.054 &  0.184 & 0.005 & 0.000 & 0.031 &  0.000 \\
    $[54000;56000]$ & $1.104(+/-)0.262$ & 0.030 &  0.258 & 0.000 & 0.000 & 0.000 &  0.036 \\
    $[56000;58000]$ & $1.064(+/-)0.053$ & 0.031 &  0.037 & 0.000 & 0.013 & 0.016 &  0.000 \\
    $[58000;60000]$ & $1.079(+/-)0.077$ & 0.035 &  0.000 & 0.043 & 0.000 & 0.000 &  0.053 \\
    $[60000;62000]$ & $1.113(+/-)0.078$ & 0.029 &  0.000 & 0.000 & 0.000 & 0.000 &  0.073 \\
    $[62000;65000]$ & $1.088(+/-)0.136$ & 0.027 &  0.123 & 0.000 & 0.000 & 0.026 &  0.043 \\
    $[65000;70000]$ & $1.077(+/-)0.074$ & 0.021 &  0.039 & 0.007 & 0.000 & 0.000 &  0.059 \\
    $[70000;80000]$ & $1.074(+/-)0.029$ & 0.015 &  0.000 & 0.000 & 0.000 & 0.000 &  0.024 \\
    $[80000;100000]$ & $1.099(+/-)0.051$ & 0.012 &  0.041 & 0.000 & 0.000 & 0.000 &  0.027 \\
    $[100000;200000]$ & $1.097(+/-)0.032$ & 0.008 &  0.000 & 0.013 & 0.000 & 0.021 &  0.019 \\
    \hline
  \end{tabular}
  \caption{Scale factor and total error as a function of $p_T$, for year 2023. The different contributions of statistical and systematic errors are shown.}
  \label{tab:scf-results-2023}
\end{table}
...
\begin{table}[h]
  ...
  \begin{tabular}{|c|c|c|c|c|c|c|c|}
    ...
    $p_T$ [GeV] & scf $\pm$ tot err & stat & channel & $E_T^{\text{miss}}(\gamma\mu)$ & $m_T(\gamma\mu)$ &  $E_T^{\text{miss}}(e)$ & $m_T(e)$ \\
    \hline
    $[50000;52000]$ & $1.318(+/-)0.166$ & 0.059 &  0.129 & 0.000 & 0.000 & 0.086 &  0.000 \\
    $[52000;54000]$ & $1.103(+/-)0.069$ & 0.026 &  0.055 & 0.000 & 0.000 & 0.019 &  0.027 \\
    $[54000;56000]$ & $1.118(+/-)0.057$ & 0.021 &  0.024 & 0.000 & 0.000 & 0.040 &  0.026 \\
    $[56000;58000]$ & $1.134(+/-)0.076$ & 0.019 &  0.018 & 0.015 & 0.000 & 0.064 &  0.027 \\
    $[58000;60000]$ & $1.107(+/-)0.022$ & 0.018 &  0.000 & 0.000 & 0.000 & 0.000 &  0.013 \\
    $[60000;62000]$ & $1.129(+/-)0.071$ & 0.018 &  0.039 & 0.000 & 0.000 & 0.049 &  0.028 \\
    $[62000;65000]$ & $1.137(+/-)0.071$ & 0.014 &  0.033 & 0.000 & 0.000 & 0.048 &  0.038 \\
    $[65000;70000]$ & $1.105(+/-)0.059$ & 0.010 &  0.023 & 0.013 & 0.000 & 0.041 &  0.031 \\
    $[70000;80000]$ & $1.125(+/-)0.054$ & 0.008 &  0.000 & 0.000 & 0.000 & 0.030 &  0.044 \\
    $[80000;100000]$ & $1.123(+/-)0.050$ & 0.006 &  0.003 & 0.008 & 0.000 & 0.018 &  0.045 \\
    $[100000;200000]$ & $1.109(+/-)0.032$ & 0.004 &  0.003 & 0.007 & 0.000 & 0.013 &  0.027 \\
    \hline
  \end{tabular}
  \caption{Scale factor and total error as a function of $p_T$, for year 2024. The different contributions of statistical and systematic errors are shown.}
  \label{tab:scf-results-2024}
\end{table}
...
The requirement $|\eta^\gamma| < 1.75$ is not applied, in order to enhance the background contribution. \\
The signal contamination is estimated assuming a ggF signal with branching ratio $BR = 1\%$.
In Figure~\ref{fig:mT-contamination}, the signal and background contributions in the transverse mass slice $80~\mathrm{GeV} < m_T < 90~\mathrm{GeV}$ are shown, corresponding to a signal fraction below 1\%. 
The signal contamination for the other kinematic variables are provided in the appendix. \\

\begin{figure} [H]%[!htbp] t!
    \centering
    \includegraphics[width=0.45\textwidth]{../images/dark_photon/images/LowMTVR/mt_lowMTVR_contamination.png}
    \caption{Signal contamination in the low \mT\ region.}
    \label{fig:mT-contamination}
\end{figure}

Given the negligible signal contamination, the total background prediction can be compared to data in the VR. Figure \ref{fig:efy_VR_mt} shows perfect agreement between the estimated background and data in the \mT\ distribution, for both 2023 and 2024. The other kinematic variables are shown in appendix \ref{sec:efy-appendix}.
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.45\textwidth]{../images/dark_photon/images/LowMTVR/mt_lowMTVRData_.png} \\
  \includegraphics[width=0.45\textwidth]{../images/dark_photon/images/LowMTVR/mt_lowMTVRData_mc23d.png} 
  \includegraphics[width=0.45\textwidth]{../images/dark_photon/images/LowMTVR/mt_lowMTVRData_mc23e.png} \\
  \caption{Comparison between data and background in the low \mT\ VR. The \mT\ distribution is shown for the full 2023+2024 dataset (top plot) and for 2023 or 2024 separately (bottom plots). Both \efakey\ and \jfakey\ background are estimated data-driven.}
  \label{fig:efy_VR_mt}
\end{figure}
