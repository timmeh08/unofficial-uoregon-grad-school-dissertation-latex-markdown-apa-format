...
\begin{itemize}
    ...
    \item the expected yield (MC) for the $\gamma$+jets (direct) background. This is a sub-dominant background, therefore no k-factor is applied; 
    \item the expected yields for electrons faking photons and jets faking photons background. 
\end{itemize}
The transverse mass (\mT) of the system represented by the visible photon and the missing transverse momentum is taken as discriminating variable between signal and background. Its spectrum is indeed expected to be peaked at $m_H\sim\SI{125}{GeV}$ for the signal, while it has a different distribution for the background processes. Data are thus split in different \mT bins:

\begin{equation}
   \mu^{[R,i]}=\mu_s^{[R,i]}+\sum_b \mu_b^{[R,i]} 
\end{equation}
\noindent
$i$ being the number of \mT bins. The binning has been optimized in order to limit the impact of statistical uncertainties while keeping the signal sensitivity of each bin as high as possible. The chosen \mT binning is the following: 
\begin{itemize}
    \item (80-110) $\SI{}{GeV}$, the lowest edge at $\SI{80}{GeV}$ is set by the cut of the trigger and of the offline selections defining the SR. This bin is dominated by \efakey and helps constraining this background; 
    \item (110-140) $\SI{}{GeV}$, the most sensitive bin centred on the Higgs mass, where signal distribution is expected to be peaked; 
    \item (140-200) $\SI{}{GeV}$, defined to contain most of the remaining signal that is not in the central bin;
    \item (200-300) $\SI{}{GeV}$, the last bin, depleted of signal, used for the normalization of the background. Together with the previous bin, it can provide some shape discrimination between \jfakey and true photons, as well as between signal and backgrounds. 
\end{itemize}
This binning is used for SR, OneMuCR and lowMETsigCR, while for TwoMuCR, due to lack of statistic in the region, it has been chosen to use only one m${_T}$ bin (80-300) GeV, in order to limit statistical uncertainties.
...
\textbf{JOB} & Luminosity & 134.6 fb$^{-1}$ \\
...
%The effect of the vertex BDT on top of the SR selections and its integration in the fit is observed to be relatively negligible. The importance of it is mostly making sure to have the "cleanest" possible dataset in terms of vertex identification, especially for the validation of the jet faking photon estimate. 
The fit takes into consideration the uncertainties on the k-factors, the systematic uncertainties on the data-driven $j \rightsquigarrow \gamma$ and $e \rightsquigarrow \gamma$ estimations, together with the MC experimental and theoretical systematic uncertainties. \\
Each source of uncertainty is treated as correlated among the analysis regions, and MC experimental systematics are also correlated among the different processes ($Z\gamma$ and $W\gamma$). A pruning threshold of 1\% is applied, acting separately on the normalization and shape effects of the various Nuisance Parameters. The normalization component of theoretical uncertainty on the $W\gamma$ and $Z\gamma$ backgrounds is not taken into account, to avoid strong correlation with the corresponding normalization factors (see appendix \ref{sec:noDropNorm_appendix}).\\
