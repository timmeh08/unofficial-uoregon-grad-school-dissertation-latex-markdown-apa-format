...
Various sources of systematic uncertainties are taken into account. These can be divided into three categories: instrumental uncertainties, theoretical uncertainties, and uncertainties related to the data-driven background estimates. The first two sources of uncertainty, applied only to MC simulations of the signal and the $Z\gamma$, $W\gamma$ contributions, are described in the following sections, while the latter is described in details in section \ref{sec:fakephotonintro}. \\
...
Table \ref{tab:impacts} reports the impact of each category of uncertainty, including statistical ones, on the signal strength, obtained via the matrix element decomposition method. 
\\
...
\begin{table}[!htbp]
  ...
  \center
    \begin{tabular}{ c | c  }
      \hline
      \textbf{Category}         &      \textbf{Contribution to uncertainty on $\mu$ (\% wrt total)}\\ \hline
      \textbf{Total error} & 0.005 \\ \hline
      \textbf{Statistical} & 80\% \\ 
      Data & 65\%\\
      MC & 47\%\\\hline
      \textbf{Experimental systematic} & 34\%\\
      Jets pile-up & 22\%\\
      Jets & 18\%\\
      $\efakey$ & 13\%\\
      $\jfakey$ & 11\%\\
      ...
      Muons &  2.3\% \\
      MET & <1\% \\\hline
      \textbf{Theoretical systematic} & 11\% \\
      \hline
    \end{tabular}
...
The integrated luminosity is measured with a precision of 2\%~\cite{lumi}, combining measurements from the LUCID-2 detector and complementary information from Inner Detector and calorimeters. This uncertainty is applied to all MC samples, including the signal and background processes, as a global normalization uncertainty. The uncertainty is treated as correlated among all analysis regions and samples.
...
The NPs \texttt{EG\_SCALE} and \texttt{EG\_RESOLUTION\_ALL} account for the uncertainty on photon energy scale and resolution. The \texttt{es2024\_Run3\_v0} ESModel, in the simplified correlation scheme \texttt{1NP\_v1} resulting in one parameter for the scale uncertainty and one for the resolution one. 
...
\begin{table}[!htbp]
  ...
  \center
    \begin{tabular}{ c | c  | c | c}
      \hline
      \textbf{Name}         &      \textbf{Description of the uncertainty} &  \textbf{\makecell{ Corr. \\ regions} }	  &  \textbf{\makecell{ Corr. \\ samples} }\\ \hline
      gamma\_stat\_REGION     & limited MC statistics in each analysis region       & no   & MC samples   \\ \hline
      ...
      scf\_ey  &  uncertainty associated to trigger scale factor in probe-e CR  & yes  & no  \\ 
