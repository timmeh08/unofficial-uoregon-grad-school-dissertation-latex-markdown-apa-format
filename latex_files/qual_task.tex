The LHC will undergo a series of upgrades during long shutdown 3, this High Luminosity LHC (HL-LHC) is planned to begin operations in 2030. 
The upgrades will aid in providing experiments with 10 times the integrated luminosity compared to previous runs after 12 years of HL-LHC data taking, with instantaneous luminosities in the range of $\mathcal{L}=5-7.5 \times 10^{34}$ cm$^{-2}$ s$^{-1}$ and approximately 200 interactions per bunch crossing.
...
The TDAQ system, dataflow from the detectors to permanent storage of events, will receive a complete overhaul in order to take full advantage of the higher pileup and manage unprecedented data rates \cite{ATLAS-TDR-29}.
Changes to the LAr and Tile readout system are thus necessary to achieve full granularity calorimeter information at the trigger level \cite{ATLAS-TDR-27, ATLAS-TDR-28}.\\
...
\begin{figure}[h!]
    ...
    \caption{Illustration of TDAQ with emphasis on the Global Trigger System.}
    \label{tdaq_global}
\end{figure}

The following upgrade sections will discuss the development and preformance studies of the topological clustering algorithm to be implemented in the Global Trigger as part of the qualification task.
...
\begin{equation}\label{N}
     |\varsigma^{EM}_{cell}| > N \textrm{   threshold for growth control}
\end{equation}
...
This qualification task has the added benefit of conducting some of these studies as a by product, and the results are discussed in Sections \ref{physics_perf} and \ref{trigger_perf}.\\
...
422-like algorithms output less clusters per event and less cells per cluster than the 420 scheme, and is more ideal for the Global Trigger.
The need for a comprehensive split/merge step for the global trigger clustering algorithm was questioned, with splitting removed entirely in the current version.
...
This qualification task will give recommendations for the firmware topoclustering algorithm used within the Global Trigger, using the USCA algorithm,based on physics performance and constraints. 

\subsection{Unique Seed Cell Association Algorithm}
...
The GTP framework was created by Maria Mazza and collaborators from Michigan State University for developers to run performance studies on custom trigger algorithms.
...
The framework can be setup following instructions from the wiki page\footnote{\href{https://twiki.cern.ch/twiki/bin/viewauth/Atlas/GlobalTriggerPerformance }{\normalfont \slshape https://twiki.cern.ch/twiki/bin/viewauth/Atlas/GlobalTriggerPerformance }}.
The USCA algorithm is currently stored on a separate fork of {\fontfamily{cmtt}\selectfont athena} 21.9.
With release 21 of {\fontfamily{cmtt}\selectfont athena} being deprecated in 2024, the GTP framework is in the process of being moved to release 24.
...
Note that new samples will be produced for the release 24 GTP framework.
All code for the analysis and plots in this paper is stored on ATLAS gitlab repositories, topo\_perf\footnote{\href{https://gitlab.cern.ch/tmathew/topo_perf}{Tim Mathew gitlab repository}}and topo\_code\footnote{\href{https://gitlab.cern.ch/rstuve/topo_code}{Ryan Stuve gitlab repository}}. 
...
There is also a cluster $E_T$ cut of 1 GeV applied to all clusters before they are sent to other algorithms.
The motivation of this energy cut is to improve the jet reconstruction performance and reduces background noise and pile-up.  
This cut is applied before splitting for the 420 and 422 clusters, and cannot be changed currently because of how the GTP framework interfaces with {\fontfamily{cmtt}\selectfont athena} algorithms.
The two candidate versions of the USCA clusters are with 2 and 3 shells, which is reflected in the performance plots. 
...
USCA cluster total $E_T$ closely matches the 422 clusters and is lower than the 420 as expected.
Without the edge $0 < E_T < 2\sigma$ cells, the 422 and USCA clusters will be a few hundred GeV less than the 420 with $\sqrt{s} = 14$ TeV collisions, as seen in the plot.
...
Where the clusters are located is a major part of jet reconstruction and accurately capturing detector response to particle showers.
The USCA cluster $\eta$ and $\phi$ are taken from the highest $E_T$ cell, which is close, but not precisely the location of 422 or 420 clusters which use the cluster moment calculations.
...
After looking into the leading 50 cluster energies in more detail, the main differences between algorithms come from which clusters make the cut in the leading 50.
Specifically, clusters near the 50 cut-off may be ordered different for each algorithm.
...
Multiplicity is shown for signal and background in Fig. \ref{cluster_mult}.
It is clearly seen that even without a splitting step, the multiplicity is almost identical between 422 and USCA clusters.
In this case, the gauge for similarity was taken to be the gaussian mean of the distributions.
...
The USCA clusters again have distributions that are similar to the 422, with the motivation to not use 420 clusters becoming clear with the much higher number of cells per cluster.
This coupled with the cluster multiplicity gives the firmware engineers concrete numbers to estimate bandwidth usage and computation time. 
...
Signal events may also not create physically large clusters, the particles tend to be collimated into a smaller R regions with high energy deposits in a smaller number of cells.
...
Future studies should use the USCA clusters as inputs to the Global Trigger jet reconstruction algorithm candidates, such as the cone jets algorithm implemented by Maria Mazza.
...
The USCA and 422 jet $p_T$ from JZ0 events diverge from their 420 counterparts, distributions are noticeably different in the 24-125 GeV range. 
...
The $\eta$ of reconstructed jets can be seen in Fig. \ref{jet_eta}, and again the distributions are similar between USCA and 422, with the 420 jets having a flatter profile because there is more splitting, allowing for jet locations that span the physical detector gaps.
These dips are seen in the cluster $\eta$ plots as well, and are a direct consequence from the cluster positions.
...
USCA jets perform similarly to the 422 jets, with slightly steeper turn-ons for the 3-shell algorithm.
...
Plots with higher shell counts are in appendix blah, and show diminishing returns for 4- and 5-shell USCA clusters.
...
For commissioning the Global Trigger, it is suggested to build and use seed-only clusters for downstream algorithms for simplicity.
...
The single jet trigger with an online threshold of 50 GeV should be efficient enough to reconstruct jets from signal events with offline $p_T$'s of 115 GeV or above.
...
Our stand-alone {\fontfamily{cmtt}\selectfont python} package \footnote{\href{https://gitlab.cern.ch/rstuve/topo_code.git }{\normalfont \slshape https://gitlab.cern.ch/rstuve/topo\_code.git }} encodes the cell $E_T$ into 10-bit binary strings following Table \ref{binary}, and the cell IDs (which contains position information) into 32-bit strings organized by event and position in $\eta$ and $\phi$ space.
...
The lookup table of neighbors is stored on the Oregon tier 3 computing cluster:/export/home/topo/neighbors.
...
The lookup table is also stored on the Oregon computing cluster:/export/home/topo/neighbors/GEP\_cell\_IDs.
...
As seen in Fig. \ref{cells_fiber}, most fibers have less than 32 $2\sigma$ cells on average for the background sample.
There are even less $4\sigma$ cells per fiber, but there are some fibers well above 32.
...
The same plots are shown in Fig. \ref{cells_fiber2} for the HH $\rightarrow$ 4b sample, with notably less cells per fiber with and without the jet $p_T$ cut.
...
Work is currently being done by the firmware engineers to make a working version of this algorithm, aided by the test vectors provided.

Future work needs to be done on the clustering algorithm to fully understand the physics performance and there is room to further optimize the software version algorithm.
The highest priority item is to integrate the USCA algorithm into the GTP package within release 24 of {\fontfamily{cmtt}\selectfont athena}. 
