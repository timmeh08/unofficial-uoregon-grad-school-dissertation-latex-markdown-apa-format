...
The LAr gap between a rod and tube contains a PEEK spacer to maintain a gap of ~260 $\mu$m in the first module and ~500 $\mu$m in the other two modules for fast and uniform signal collection.
A dense absorber with matrix cells is necessary to contain the high energy particle showers in this region, with interaction lengths of ~8 in the copper module and ~10 in the tungsten modules.
...
At the LHC bunch crossing rate of 40 MHz, ATLAS produces an enormous amount of data, over 1 PB per second if all detector information were to be recorded.
...
The ATLAS Trigger and Data Acquisition (TDAQ) system \cite{tdaq_2016} utilizes a two level trigger system to reduce the 40 MHz LHC bunch crossing rate to a manageable 1 kHz for offline storage and analysis.
...
Within the L1 system, new feature extractors (FEX) boards exploit the higher granularity calorimeter data to perform more advanced object reconstruction and identification.
These include the eFEX for electrons, photons, and taus; jFEX for jets, taus, and $E_T^{\text{miss}}$; gFEX for large-R jets, $E_T^{\text{miss}}$, and $H_T$.
...
New software support of multi-threading within the HLT allows for operation at higher event rates without reaching computing resource limits.