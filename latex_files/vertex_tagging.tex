...
\begin{figure}[htbp]
    ...
    \caption{Predicted probabilities from the neural network classifier for candidates split into correctly identified vertices and incorrectly identified vertices, for both the training with preselections (bottom) and without preselections (top), and split by background (left) and signal (right). The classifier shows good separation between correctly identified vertices and incorrectly identified vertices, with correctly identified vertices having higher predicted probabilities.}
    \label{probabilities}
\end{figure}

The final performance of the classifier is evaluated using a simple accuracy metric, which is the fraction of events where the primary vertex is correctly identified.
