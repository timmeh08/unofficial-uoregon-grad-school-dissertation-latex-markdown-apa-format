...
Recent measurements of the W boson mass, which initially showed a significant deviation from the SM prediction, have been updated with new measurements that are consistent with the SM \cite{science.abk1781,atlas_2024}, further confirming that the SM is able to withstand intense scrutiny.
...
Additionally, at certain energy scales, the SM requiresneutrinos to be massless, which is contradicted by neutrino oscillation observations.
...
Table \ref{sm_charges} summarizes the gauge representations of the SM fermions under these symmetry groups.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|c|c|c|c|}
        ...
        \hline
    \end{tabular}
    \caption{Gauge representations of the Standard Model fermion fields. The three fermion generations are not shown, as all generations have the same quantum numbers.}
    \label{sm_charges}
\end{table}


\subsection{Electroweak Symmetry Breaking and the Higgs Mechanism}\label{sec:ewsb}
...
which has stationary points at $\phi^\dagger \phi = 0, -\mu^2/\lambda$.
...
The $W^3$ boson mixes with the $B$ boson to give the physical $Z$ and $\gamma$ bosons, while the $W^1$ and $W^2$ bosons combine to form the $W^{\pm}$ bosons.
...
The observation of the Bullet Cluster, where galaxies have collided and the dark matter component has been observed through gravitational lensing and is separate from the baryonic matter, provides clear evidence for the existence of dark matter.
...
\begin{figure}[htbp]
    ...
    \caption{The observed rotation curve of galaxy M33 compared to the expected rotation curve from visible matter \cite{corbelli2000extended} (left) and the Bullet Cluster showing the separation of dark matter and baryonic matter through gravitational lensing \cite{Massey_2010} (right).}
...
A universe without dark matter would not have the necessary gravitational potential wells to form galaxies and other large scale structures within the time scales of our universe, and the observed distribution of galaxies match simulations incorporating $\Lambda$CDM cosmology.
...
The Higgs boson is a prime candidate for such a portal, given its unique properties, coupling to mass, and the current constraints on the Higgs to invisible branching ratio.
...
These dark mesons are previously unconstrained by existing searches due to the nature of BSM searches that typically require high MET or high mass resonances while the dark meson mass regime is lower.
...
If $\eta < 0.5$, the dark rho can decay to two dark pions with a branching fraction near 1, while if $\eta > 0.5$, the dark rho decays to pairs of SM fermions.
...
Another minimal extension to the Standard Model, that has gained traction in recent years, and allows access to a dark sector is through the introduction of a new $U(1)_D$ gauge symmetry, with the associated gauge boson being the dark photon, $\gamma_D$.
...
This vev is similar to the Higgs vev described in Section \ref{sec:ewsb}, and allows for the messenger fields to acquire mass and generate the necessary interactions between the dark photon and the SM.
Once the scalar field $S_0$ acquires a vev, the messenger fields acquire mass and generate trilinear couplings between the dark photon and the SM Higgs boson at one loop level, $H\gamma\gamma_D$ and $H\gamma_D\gamma_D$, with couplings proportional to $\mu_S=\lambda_S<S_0>$.
